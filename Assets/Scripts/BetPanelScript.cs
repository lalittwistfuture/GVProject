...
namespace Roullet
{
    public class BetPanelScript : MonoBehaviour
    {
        ...
        {
            Debug.Log((PlayerPrefs.GetInt(RouletteTag.TOTAL_BET) + PlayerPrefs.GetInt(RouletteTag.RouletteSelectedCoin)));
            if (Enable)
            {
                Debug.Log("Working " + PlayerPrefs.GetInt(RouletteTag.RouletteSelectedCoin));
                if (PlayerPrefs.GetInt(RouletteTag.RouletteSelectedCoin) > 0 && (int.Parse(PlayerPrefs.GetString(PlayerDetails.Coin)) >= PlayerPrefs.GetInt(RouletteTag.RouletteSelectedCoin)))
                {
                    ...
                    if (PlayerPrefs.GetInt(RouletteTag.RouletteSelectedCoin) > 0)
                    {
                        this.row = r;
                        this.column = c;
                        this.side = s;
