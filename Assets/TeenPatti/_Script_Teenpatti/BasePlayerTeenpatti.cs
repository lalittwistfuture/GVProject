...
public class BasePlayerTeenpatti : MonoBehaviour {
    ...
    public void updateUserData (JSONNode playerData1) {
        ...
        try {
            string coin1 = playerData1[ServerTagsTeenpatti.TOTAL_COINS];
            string coin2 = playerData1[ServerTagsTeenpatti.BET_COIN];
            if ((coin1 != null) && (coin2 != null)) {
                this.coin = int.Parse (coin1);
    ...
    private void CollectBootAmt (JSONNode node) {
        ...
        if (player_id.Equals (playe)) {
            totalCoin.text = node[PlayerTagsTeenPatti.TOTAL_COINS];
            if (player_id.Equals (PlayerPrefs.GetString (PrefebTagsTeenpatti.PLAYER_ID))) {
                if (int.Parse (node[ServerTagsTeenpatti.BOOT_AMOUNT]) <= this.coin) {
    ...
    public void ChangePlayerStatus () {
        ...
        switch (this.Status) {
            ...
            case 3:
                {
                    StatusMsg ("Your do not have sufficient coin");
                    UserInactive ("Pack");
    ...
    public virtual void UpdateCoin (int TotalBetAmt) {

    }

    public virtual void DeductBootAmt (int TotalBetAmt) {

    }

    public virtual void UpdateRoomInfo (int TotalCoin) {
